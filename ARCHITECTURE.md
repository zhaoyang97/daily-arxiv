...
- **关注领域**: 15 个核心领域（VLM、LLM推理、Agent、图像生成、3D视觉等）
- **打分维度**: 领域权重(0-20) + 团队知名度(0-15) + 高价值关键词(0-30) + 代码可用(0-5) + 摘要质量(0-10) - 低价值惩罚(-10)
- **A 档** (默认 20 篇): 得分最高，写深度笔记（读全文，100+ 行）
- **B 档** (默认 40 篇): 次高分，写轻量笔记（只读 abstract，50 行）
- 支持 `--top-a` / `--top-b` 自定义数量
