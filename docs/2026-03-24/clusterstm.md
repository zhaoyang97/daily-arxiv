...
视频帧 → 视觉编码 → 帧内语义聚类（将token分为K个语义组）→ 每个组内按时间密度排序保留top-1 token → 保留的稀疏token做视频语言联合建模 → 视频-文本相关性重建作为额外训练目标。
...
- 训练效率保持在 ~50%（与随机遮掩相当），语义聚类增加的开销可忽略- 聚类数 K=8 最优，K=4 时覆盖不足，K=16 时每类 token 太少导致密度估计不稳定
