...
| M | FLAT 中位读次 | INDEXED | 理论比 M/(2P) | 实际比 |
|---|------------|---------|------------|--------|
| 50 | 3.0 | 1.0 | 2.5× | 3× |
| 100 | 6.0 | 1.0 | 5× | 6× |
| 200 | 9.0 | 1.0 | 10× | 9× |
...
| M | FLAT tokens | INDEXED tokens | 比值 |
|---|------------|---------------|------|
| 1000 | 174K | 6K | 29× |
| 2000 | **914K** | **6K** | **153.6×** |
...
|------|-------------|--------|------|
| FLAT | 21.0 | 96% | 线性扫描 |
| FLAT-SORTED (GPT-5.4) | 5.0 | 96% | 接近二分搜索但仍 5× 于索引 |
| INDEXED | **1.0** | **96%** | B-tree 索引 |
| INDEXED-CORRUPTED | 11.0-15.5 | **58%** | 打乱索引崩溃→证明因果使用 |
| DEEP-INDEXED (2-level) | ~1.0 | 96% | M=5000 仍保持 O(1) |
...
## 相关工作与启发
- **vs RAG 系统**: RAG 用向量检索做“近似匹配”；本文证明精确检索下 B-tree 指数优于线性扫描
- **vs 记忆增强 Agent (MemGPT)**: MemGPT 用层级存储但未形式化复杂度；本文提供理论基础
- **vs 长上下文模型**: 超长上下文仍是线性扫描；索引化外部记忆是正交补充

## 评分
- 新颖性: ⭐⭐⭐⭐⭐ 将经典 B-tree 复杂度理论引入 Agent 系统设计
- 实验充分度: ⭐⭐⭐⭐ 精心设计的控制实验、因果验证、参数记忆竞争现象
- 写作质量: ⭐⭐⭐⭐⭐ 理论证明 + 实验完美匹配理论预测
